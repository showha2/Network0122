            ...
            if(msg.what == 1){
                ...
                var path =
                    Environment.getDataDirectory().absolutePath +
                            "/data/itstudy.kakao.network/files/" +
                            filename
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnImageDisplay.setOnClickListener{
            object:Thread(){
                override fun run(){
                    //스트림 생성
                    val inputStream = URL(
                        "https://img.huffingtonpost.com/asset/600628c12500003400e2332a.jpeg?cache=6LdRxGcV9t&ops=crop_0_314_550_303,scalefit_630_noupscale")
                        .openStream()
                    //비트맵 가져오기
        ...
        btnImageSave.setOnClickListener{
            ...
            var path = Environment.getDataDirectory().absolutePath +
                    "/data/itstudy.kakao.network/files/30.jpeg"
            //파일 존재 여부 조사
            if(File(path).exists()){
                //있는 경우에는 바로 출력
                Toast.makeText(this,
                    "파일이 존재 - 다운로드 받지 않음", Toast.LENGTH_LONG).show()
                ...
                //없는 경우에는 다운로드 받아서 출력
                Toast.makeText(this,
                    "파일이 없음 - 다운로드 받아서 출력", Toast.LENGTH_LONG).show()
                ...
                object:Thread(){
                    override fun run(){
                        ...
                        //파일 출력을 위한 스트림 생성
                        val fos = openFileOutput("30.jpeg", 0)
                        //읽어서 기록
                        while(true){
                            var len = inputStream.read(raster)
